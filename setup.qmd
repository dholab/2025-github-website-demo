...
5. Keep in mind that as currently configured, the website-update action will run _any time any file in main is updated_. If you're pushing lots of little updates, it may make sense to either a) temporarily disable the publishing action, or b) restrain the action trigger to only when particular files, e.g., particular `.qmd` or `.py` files, are updated.
